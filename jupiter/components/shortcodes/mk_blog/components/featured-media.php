...
switch ($view_params['post_type']) {
    case 'image':
        if (!Mk_Image_Resize::is_default_thumb($image_src_array[0])) {
            echo '<div class="featured-image tt"><span class="pub">'. get_the_date( 'd M Y' ).'</span><a class="full-cover-link '.$image_permalink_class.'" title="' .  the_title_attribute(array('echo' => false)) . '" href="' . $image_permalink . '">&nbsp;</a>';
